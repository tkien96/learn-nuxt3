import {  MCreate, MReadUnique } from "~/server/models"
...
export default defineEventHandler(async (event) => {
    try {
        ...
        const checkUnique = await MReadUnique("users", { select: ['id'] }, { where: { OR: [{ email: email }, { phone: phone }] } });
        ...
        return {  status: 200, message: 'Register Successfully !', data: await tranformerUser(user) }
    } catch (error) {
